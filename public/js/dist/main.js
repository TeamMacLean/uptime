...
},{"../core/core.defaults":"../../../node_modules/chart.js/src/core/core.defaults.js","../helpers/index":"../../../node_modules/chart.js/src/helpers/index.js","../core/core.scaleService":"../../../node_modules/chart.js/src/core/core.scaleService.js","../core/core.ticks":"../../../node_modules/chart.js/src/core/core.ticks.js"}],"../../../node_modules/chart.js/src/scales/scale.time.js":[function(require,module,exports) {
/* global window: false */
'use strict';
...
window.charts = [];
window.dataSets = [];
var apdexT = 0.2; // import {apdexT} from '../../../config.js';

var apdexTInMS = apdexT * 1000;
...
window.loadGraph = function (siteID, range) {
  //mark as active
  var links = document.getElementById("card-" + siteID).getElementsByClassName('graph')[0].getElementsByClassName('link');
...
function Queue() {
  ...
  var queue = []; // We're inactive to begin with

  queue.active = false; // Method for adding command chain to the queue

  queue.place = function (command) {
    // Push the command onto the command array
    queue.push(command); // If we're currently inactive, start processing
  ...
  queue.next = function () {
    ...
    if (!queue.length) {
      ...
      queue.active = false; // Stop execution

      return;
    } // Grab the next command


    var command = queue.shift(); // We're active

    queue.active = true; // Call the command
...
window.queue = new Queue(); // const safe = '#2CDCBE';
// const warn = '#24B1DD';
// const danger = '#725EFB';
...
window.buildGraph = function (name, responses) {
  ...
  var quickData = responses.reduce(function (all, r, idx) {
    all.labels.push((0, _moment.default)(r.createdAt).calendar());
  ...
  if (window.charts[name]) {
    ...
    window.charts[name] = new _chart.default(ctx, {
      type: 'line',
      //'bar',
      data: processedData,
      options: {
      ...
      plugins: [{
        ...
        afterLayout: function afterLayout(chart, options) {
          var scales = chart.scales; // create a linear gradient with the dimentions of the scale

          var color = chart.ctx.createLinearGradient(0, //scales["x-axis-0"].left,
          scales["y-axis-0"].bottom, 0, //scales["x-axis-0"].right,
          scales["y-axis-0"].top); //vertical

          var max = Math.max.apply(Math, _toConsumableArray(chart.data.datasets[0].data));
          var bit = max > 0 ? 1 / max : 0;
          ...
          chart.data.datasets[0].borderColor = color; // chart.data.datasets[0].backgroundColor = color;
        }
      }]
    });
  }
